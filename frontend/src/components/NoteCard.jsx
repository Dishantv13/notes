...
const NoteCard = ({ note, onEdit, onDelete, onTogglePin, onView }) => {
  return (
    ...
    >
      ...
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-slate-900 pr-8 line-clamp-1">{note.title}</h3>
      ...
      <div className="mt-6 flex flex-wrap gap-1.5">
        {note.tags.map((tag, i) => (
          <span key={i} className="flex items-center gap-1 rounded-md bg-slate-100 px-2 py-0.5 text-[10px] font-medium text-slate-600 border border-slate-200">
            <Tag size={10} />
            {tag}
          </span>
        ))}
